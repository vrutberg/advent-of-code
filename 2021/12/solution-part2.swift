...
func makeNode(
    ...
    cave.caves.forEach { cave in
        switch cave.id {
        case .small:
            if path.contains(cave.id) {
                let smallCaves = path.filter(\.isSmall)
                if Set(smallCaves).count == smallCaves.count {
                    subNodes.append(makeNode(cave: cave, path: path + [cave.id]))    
                }
            } else {
                subNodes.append(makeNode(cave: cave, path: path + [cave.id]))
            }
        ...
        case .start:
            break
        case .end:
            subNodes.append(Node(cave: cave.id, children: []))
        }
    }
