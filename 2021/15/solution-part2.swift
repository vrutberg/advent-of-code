...
let emptyRow = [Int](repeating: -1, count: lines.count * 5)
var newLines = [[Int]](repeating: emptyRow, count: lines.count * 5)
let width = lines.count

(0..<5).forEach { xFactor in 
    (0..<5).forEach { yFactor in
        lines.enumerated().forEach { y, row in
            row.enumerated().forEach { x, value in
                let newX = ((xFactor) * lines.count) + x
                let newY = ((yFactor) * lines.count) + y
                let newValue = (value + xFactor + yFactor) % 9
                newLines[newY][newX] = newValue == 0 ? 9 : newValue
            }
        }
    }
}
