...
let result = folds.reduce(dots, fold)
let maxX = result.sorted(by: { $0.x > $1.x }).first!.x
let maxY = result.sorted(by: { $0.y > $1.y }).first!.y

(0...maxY).map { y -> String in
    String((0...maxX).map { x -> Character in
         result.contains(Coordinate(x: x, y: y)) ? "#" : " "
    })
}
.forEach { print($0) }